...
function relatedWaUrl(carta) {
  const text = `Olá, Felipe! Tenho interesse na carta ${carta.code || ''} da ${carta.administrator || ''}. Crédito: ${brl(carta.credit)}. Parcela: ${brl(carta.installment)}. Gostaria de receber mais informações.`.trim();
...
function buildHtml(carta) {
  ...
  const relatedSection = related.length ? `<tr><td style="padding:24px 26px 8px;border-top:1px solid #e4ebec"><div style="font:700 18px Georgia,serif;color:#173b48">Mais opções para comparar</div><div style="margin-top:5px;font:12px/1.5 Arial,sans-serif;color:#63787d">Confira outras cartas reais do catálogo e escolha a condição que mais combina com seu planejamento.</div></td></tr><tr><td style="padding:8px 26px 18px"><table role="presentation" width="100%" cellspacing="6" cellpadding="0">${related.map((x, i) => `<tr><td style="width:50%;padding:14px 13px;background:#fff;border:1px solid #e5ecee;border-top:4px solid ${i % 2 ? '#d6a646' : '#168c63'};border-radius:11px;vertical-align:top"><div style="font:700 9px Arial,sans-serif;letter-spacing:1px;text-transform:uppercase;color:#70858b"><span style="display:inline-block;margin-right:5px;color:${i % 2 ? '#9a6b00' : '#168c63'}">${String(i + 1).padStart(2, '0')}</span>${esc(x.administrator || 'Carta contemplada')}</div><div style="margin-top:7px;font:700 16px Georgia,serif;color:#173b48">${esc(x.code)}</div><div style="margin-top:6px;font:700 16px Arial,sans-serif;color:#168c63">${esc(brl(x.credit))}</div><div style="display:inline-block;margin-top:7px;padding:4px 7px;border-radius:5px;background:${i % 2 ? '#fff8e8' : '#eef8f3'};font:700 10px Arial,sans-serif;color:#526970">Parcela: ${esc(brl(x.installment))}</div><a href="${relatedWaUrl(x)}" style="display:inline-block;margin-top:10px;padding:7px 10px;border-radius:7px;background:#168c63;color:#fff;text-decoration:none;font:700 10px Arial,sans-serif">Falar no WhatsApp</a></td>${i % 2 ? '</tr>' : ''}`).join('')}${related.length % 2 ? '<td style="width:50%;padding:12px"></td></tr>' : ''}</table><div style="padding-top:14px;text-align:center"><a href="https://condominiosnapraia.com.br/contemplado-imoveis/" style="display:inline-block;margin:3px;padding:11px 16px;border-radius:9px;background:#123d4b;color:#fff;text-decoration:none;font:700 12px Arial,sans-serif">Ver mais opções</a><a href="${waUrl(carta)}" style="display:inline-block;margin:3px;padding:11px 16px;border-radius:9px;background:#168c63;color:#fff;text-decoration:none;font:700 12px Arial,sans-serif">Falar com consultor no WhatsApp</a></div></td></tr>` : '';
...
<tr><td style="padding:28px 26px 25px;background:#123d4b;color:#fff;text-align:center"><div style="font:700 10px Arial,sans-serif;letter-spacing:2px;color:#d7bb72;text-transform:uppercase">PORTAL MEU LITORAL · CRÉDITO IMOBILIÁRIO</div><div style="margin-top:11px;font:700 28px/1.12 Georgia,serif;letter-spacing:-.2px">Crédito contemplado<br><span style="color:#d7bb72">para comprar seu imóvel</span></div><div style="margin-top:13px;font:13px/1.5 Arial,sans-serif;color:#dce9eb">Uma opção de crédito já contemplada, apresentada para sua análise com clareza e segurança.</div></td></tr>
<tr><td style="padding:22px 26px 4px"><div style="font:700 10px Arial,sans-serif;letter-spacing:1.4px;text-transform:uppercase;color:#8b6b22">CARTA SELECIONADA</div><h1 style="margin:7px 0 4px;font:700 24px/1.15 Georgia,serif;color:#173b48">${esc(admin)}</h1><div style="font:13px Arial,sans-serif;color:#63787d">Código da carta: <strong>${esc(carta.code || 'Não informado')}</strong></div></td></tr>
<tr><td style="padding:18px 26px 8px"><div style="padding:18px;border-radius:14px;background:linear-gradient(135deg,#f8f2df,#fffaf0);border:1px solid #ead9a8"><div style="font:700 10px Arial,sans-serif;letter-spacing:1.4px;text-transform:uppercase;color:#907022">CRÉDITO CONTEMPLADO</div><div style="margin-top:5px;font:700 32px/1.1 Georgia,serif;color:#123d4b">${esc(credit)}</div><div style="margin-top:6px;font:13px/1.5 Arial,sans-serif;color:#5f7074">Valor informado no cadastro da carta contemplada e sujeito à confirmação com a administradora.</div></div></td></tr>
