...
export async function onRequest() {
  ...
  for (const site of Array.isArray(sites) ? sites : []) {
    ...
    const landing = `${SITE}/corretor/${encodeURIComponent(site.slug)}/`;
    ...
    (Array.isArray(rows) ? rows : []).forEach((row) => {
      ...
      urls.push({ loc: `${SITE}/corretor/${encodeURIComponent(site.slug)}/imovel/${encodeURIComponent(propertySlug(im))}/`, lastmod: lastmod(row) || lastmod(im), priority: '0.7' });
    });
  }
