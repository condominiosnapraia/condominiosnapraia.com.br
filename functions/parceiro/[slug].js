...
<section class="hero">${site.capa_url ? `<img class="hero-bg" src="${esc(site.capa_url)}" alt="" fetchpriority="high">` : ''}<div class="hero-inner"><div class="eyebrow">Atendimento imobiliário</div><h1>${esc(name)}</h1><p>${esc(description)}</p></div></section>
...
export async function onRequest(context) {
  ...
  const site = sites[0];
  const listings = await getJson(`parceiros_sites_imoveis?site_id=eq.${site.id}&publicado=eq.true&select=id,ordem,destaque,titulo_personalizado,chamada_personalizada,imovel:imoveis(id,slug,titulo,cidade_end,bairro,preco,quartos,suites,area,descricao,fotos,fotos_no_site,fotos_para_site,status,publicar)&order=destaque.desc,ordem.asc&limit=100`);
