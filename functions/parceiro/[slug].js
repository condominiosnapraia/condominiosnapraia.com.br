...
const SECTIONS = [
  ...
  { id: 'terreno-condominio', category: 'terreno-condominio', title: 'Terrenos', subtitle: 'Em condomínio', limit: 6, icon: '📐', empty: 'Ainda não há terrenos em condomínio publicados.' },
...
function layout({ site, properties, slug, requestPath }) {
  ...
  const logo = `<span class="brand-mark">${esc(initials)}</span>`;
...
<meta name="robots" content="index,follow"><meta property="og:type" content="website"><meta property="og:site_name" content="Condomínios na Praia"><meta property="og:title" content="${esc(pageTitle)} | Condomínios na Praia"><meta property="og:description" content="${esc(description)}">${site.capa_url ? `<meta property="og:image" content="${esc(site.capa_url)}">` : ''}
...
<footer><div class="footer-main"><div class="footer-col"><div class="footer-logo">${esc(name)}</div><p class="footer-text">Site profissional conectado ao catálogo do Condomínios na Praia, com imóveis selecionados e atendimento personalizado no Rio Grande do Sul.</p><a class="footer-wpp" href="${esc(wpp)}" target="_blank" rel="noopener nofollow">WhatsApp do corretor</a></div><div class="footer-col"><div class="footer-title">Navegar</div><a href="#buscar">Buscar imóveis</a><a href="#imoveis">Todos os destaques</a><a href="#contato">Falar com o corretor</a><a href="${BASE}/">Portal Condomínios na Praia</a></div><div class="footer-col"><div class="footer-title">Atendimento</div><span>${esc(site.creci ? `CRECI ${site.creci}` : 'Atendimento imobiliário')}</span><span>${esc(site.cidade || 'Rio Grande do Sul')}</span>${site.email ? `<a href="mailto:${esc(site.email)}">${esc(site.email)}</a>` : '<span>Atendimento pelo WhatsApp</span>'}</div><div class="footer-col"><div class="footer-title">Informações</div><a href="${BASE}/politica-privacidade/">Política de Privacidade</a><a href="${BASE}/termos/">Termos de Uso</a><a href="${BASE}/contato/">Contato do portal</a></div></div><div class="footer-bottom"><span>© 2026 ${esc(name)} · Site parceiro Condomínios na Praia</span><span>Rio Grande do Sul · Brasil</span></div></footer>
...
function runSearch(){const q=(document.getElementById('property-query').value||'').trim().toLowerCase();const cat=document.getElementById('property-category').value;const rows=PARTNER_DATA.filter(p=>(!cat||p.category===cat)&&(!q||[p.title,p.type,p.city,p.neighborhood,p.cond&&p.cond.name,p.description].filter(Boolean).join(' ').toLowerCase().includes(q))).slice(0,48);searchBox.classList.add('on');searchTitle.textContent=rows.length+' resultado'+(rows.length===1?'':'s');searchGrid.innerHTML=rows.length?rows.map(p=>clientCard(p)).join(''):'<div class="empty">Nenhum imóvel correspondeu à sua busca.</div>';wireInterest();searchBox.scrollIntoView({behavior:'smooth',block:'start'});}
searchForm.addEventListener('submit',e=>{e.preventDefault();runSearch();});document.getElementById('clear-search').addEventListener('click',()=>{document.getElementById('property-query').value='';document.getElementById('property-category').value='';searchBox.classList.remove('on');searchGrid.innerHTML='';});wireInterest();
...
export async function onRequest(context) {
  ...
  if (requestPath.startsWith('/parceiro/')) return Response.redirect(`${BASE}/corretor/${encodeURIComponent(slug)}/`, 301);
  ...
  const properties = listingRows.map((row) => normalizeListing(row, condMap, slug));
  return new Response(layout({ site, properties, slug, requestPath }), { headers: { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'public, max-age=60, s-maxage=300, stale-while-revalidate=3600' } });
}
