...
function card(property, compact = false, interestBase = '', brokerName = '') {
  const details = [
  ...
  return `<article class="property-card${compact ? ' property-card-compact' : ''}">
    ...
    <div class="property-body">
      ...
      <div class="property-actions"><a class="property-link" href="${esc(property.url)}">Ver imóvel</a><a class="property-interest" href="${esc(interestUrl)}" target="_blank" rel="noopener nofollow">Tenho interesse</a></div>
    </div>
  </article>`;
}
...
function layout({ site, properties, slug, requestPath }) {
  ...
  properties.forEach((property) => { const rawCity = String(property.city || '').trim(); const key = cityKey(rawCity); if (rawCity && key !== cityKey('Rio Grande do Sul') && !cityMap.has(key)) cityMap.set(key, cityLabel(rawCity)); });
  ...
  const sectionCounts = Object.fromEntries(SECTIONS.map((section) => [section.id, properties.filter((property) => property.category === section.category).length]));
  const categoryNav = `<nav class="category-nav" aria-label="Categorias de imóveis">${SECTIONS.map((section) => `<a class="category-chip" href="#sec-${section.id}"><span>${section.icon}</span><strong>${section.title}</strong><small>${section.subtitle} · ${sectionCounts[section.id]}</small></a>`).join('')}</nav>`;
  ...
  const sectionMarkup = SECTIONS.map((section, index) => {
    ...
    return `<section class="property-section section-tone-${index % 2 ? 'soft' : 'plain'}" id="sec-${section.id}" data-category="${section.category}"><div class="section-head"><div><span class="eyebrow">${section.icon} Oportunidades</span><h2>${section.title}<small>${section.subtitle}</small></h2></div><span class="section-count">${items.length} ${items.length === 1 ? 'imóvel' : 'imóveis'}</span></div>${items.length ? `<div class="property-grid">${items.map((item) => card(item, false, wpp, name)).join('')}</div>` : `<div class="empty">${section.empty}</div>`}</section>`;
  }).join('');
...
<style>
:root{--brand:${brand};--accent:${accent};--ink:#0d3b54;--muted:#5b7585;--sand:#faf6ef;--line:#e7ded0;--white:#fff;--soft:#eef7f7}*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:var(--sand);color:var(--ink);font-family:Outfit,Arial,sans-serif}a{text-decoration:none;color:inherit}.site-header{position:sticky;top:0;z-index:20;background:rgba(250,246,239,.94);backdrop-filter:blur(16px);border-bottom:1px solid rgba(13,59,84,.1)}.nav{position:relative;width:min(1180px,100%);margin:auto;padding:14px 24px;display:flex;align-items:center;gap:14px}.brand{display:flex;align-items:center;gap:10px;font-weight:700;letter-spacing:.03em}.brand-mark{display:grid;place-items:center;width:34px;height:34px;border-radius:10px;background:var(--brand);color:#fff;font-size:12px;letter-spacing:.1em}.brand-text{white-space:nowrap}.brand-logo{width:38px;height:38px;border-radius:10px;object-fit:cover}.nav-links{margin-left:auto;display:flex;align-items:center;gap:18px;color:var(--muted);font-size:13px}.nav-cta{padding:9px 14px;border-radius:999px;background:var(--brand);color:#fff;font-weight:600}.hero{position:relative;min-height:380px;display:grid;align-items:end;overflow:hidden;background:linear-gradient(140deg,var(--brand),#0e8a99)}.hero:after{content:'';position:absolute;inset:0;background:linear-gradient(180deg,rgba(5,34,49,.22),rgba(5,34,49,.86))}.hero-bg{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.55;filter:saturate(.88)}.hero-inner{position:relative;z-index:1;width:min(1180px,100%);margin:auto;padding:72px 24px 48px;color:#fff}.eyebrow{text-transform:uppercase;letter-spacing:.19em;color:var(--accent);font-size:11px;font-weight:700}.hero h1{max-width:820px;margin:14px 0 10px;font:600 clamp(38px,5.8vw,66px)/.98 Fraunces,serif;letter-spacing:-.04em}.hero-title-clean{margin:0!important;display:grid;gap:8px}.hero-title-clean span{display:block}.hero-title-clean span+span{font-size:.68em;letter-spacing:-.02em;color:var(--accent)}.hero p{max-width:660px;margin:0;font-size:17px;line-height:1.65;color:rgba(255,255,255,.9)}.hero-actions{display:flex;flex-wrap:wrap;gap:10px;margin-top:22px}.hero-action{display:inline-flex;align-items:center;justify-content:center;min-height:42px;padding:10px 17px;border-radius:999px;font-size:13px;font-weight:700;transition:transform .18s ease,background .18s ease}.hero-action:active{transform:scale(.97)}.hero-action.primary{background:var(--accent);color:#193c50}.hero-action.secondary{border:1px solid rgba(255,255,255,.68);color:#fff;background:rgba(255,255,255,.08)}.hero-action:hover{transform:translateY(-2px)}.hero-quick{display:flex;flex-wrap:wrap;gap:8px;margin-top:18px}.hero-quick span{border:1px solid rgba(255,255,255,.24);border-radius:999px;padding:7px 10px;background:rgba(5,34,49,.24);color:rgba(255,255,255,.84);font-size:11px}.main{width:min(1180px,100%);margin:auto;padding:30px 24px 110px}.search-panel{margin:-48px auto 24px;position:relative;z-index:3;background:#fff;border:1px solid rgba(13,59,84,.1);border-radius:24px;padding:20px;box-shadow:0 24px 60px rgba(13,59,84,.15)}.search-head{display:flex;justify-content:space-between;align-items:center;gap:14px;margin-bottom:12px}.search-head h2{font:600 28px/1 Fraunces,serif;margin:0}.search-head p{margin:0;color:var(--muted);font-size:12px}.search-form{display:grid;grid-template-columns:1.35fr .8fr .8fr .8fr .8fr auto;gap:10px}.search-form input,.search-form select{width:100%;min-height:46px;border:1px solid #dbe6e8;border-radius:12px;background:#f8fbfb;color:var(--ink);padding:12px 13px;font:14px Outfit,sans-serif;outline:none}.search-form input:focus,.search-form select:focus{border-color:var(--brand);box-shadow:0 0 0 3px rgba(13,92,134,.1);background:#fff}.search-form button{border:0;border-radius:12px;background:var(--brand);color:#fff;padding:12px 18px;font:700 13px Outfit,sans-serif;cursor:pointer;box-shadow:0 10px 18px rgba(13,92,134,.18);transition:transform .18s ease,filter .18s ease}.search-form button:hover{filter:brightness(1.08);transform:translateY(-1px)}.search-results{display:none;margin-top:18px;padding-top:18px;border-top:1px solid var(--line)}.category-nav{display:flex;gap:10px;overflow-x:auto;margin:0 0 18px;padding:2px 0 6px;scrollbar-width:none}.category-nav::-webkit-scrollbar{display:none}.category-chip{min-width:166px;display:grid;grid-template-columns:32px 1fr;grid-template-rows:auto auto;column-gap:9px;align-items:center;padding:12px 14px;border:1px solid rgba(13,59,84,.1);border-radius:16px;background:rgba(255,255,255,.74);box-shadow:0 8px 22px rgba(13,59,84,.045);transition:transform .18s ease,border-color .18s ease,background .18s ease}.category-chip:hover{transform:translateY(-2px);border-color:rgba(13,92,134,.3);background:#fff}.category-chip>span{grid-row:1/3;display:grid;place-items:center;width:32px;height:32px;border-radius:11px;background:var(--soft);font-size:16px}.category-chip strong{font-size:12px;line-height:1.15;color:var(--ink)}.category-chip small{margin-top:3px;color:var(--muted);font-size:10px;line-height:1.15}.feature-strip{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:0 0 18px;padding:14px;border-radius:20px;background:linear-gradient(105deg,rgba(13,92,134,.08),rgba(213,170,87,.11));border:1px solid rgba(13,92,134,.08)}.feature-item{display:flex;align-items:center;gap:10px;min-width:0;padding:7px 8px}.feature-icon{display:grid;place-items:center;flex:0 0 34px;width:34px;height:34px;border-radius:12px;background:#fff;color:var(--brand);font-size:19px;box-shadow:0 5px 14px rgba(13,59,84,.08)}.feature-item strong,.feature-item span{display:block}.feature-item strong{font-size:12px;line-height:1.1}.feature-item div>span{margin-top:4px;color:var(--muted);font-size:10px;line-height:1.25}.search-results.on{display:block}.search-results-head{display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:12px;color:var(--muted);font-size:13px}.search-results-head strong{color:var(--ink)}.intent-row{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin:70px 0 0}.final-intents{scroll-margin-top:90px}.intent-row a{display:flex;flex-direction:column;gap:5px;padding:18px 20px;border:1px solid var(--line);border-radius:16px;background:#fff;box-shadow:0 10px 24px rgba(13,59,84,.05);transition:transform .18s ease,box-shadow .18s ease}.intent-row a:hover{transform:translateY(-2px);box-shadow:0 14px 28px rgba(13,59,84,.1)}.intent-row strong{font:600 24px/1 Fraunces,serif;color:var(--brand)}.intent-row span{color:var(--muted);font-size:12px;line-height:1.45}.section-intro{max-width:720px;margin-bottom:10px}.sr-title{margin:7px 0 5px;font:600 clamp(30px,4.6vw,50px)/.98 Fraunces,serif;letter-spacing:-.03em}.intro-text{margin:0;color:var(--muted);font-size:14px;line-height:1.6}.property-section{padding:44px 0 38px;scroll-margin-top:90px}.property-section+.property-section{margin-top:18px;border-top:0}.property-section.section-tone-soft{margin-left:-28px;margin-right:-28px;padding:42px 28px 44px;border:1px solid rgba(13,59,84,.07);border-radius:28px;background:rgba(255,255,255,.58)}.section-head{display:flex;align-items:end;justify-content:space-between;gap:20px;margin-bottom:22px}.section-head h2{margin:9px 0 0;font:600 clamp(30px,4.6vw,50px)/.98 Fraunces,serif;letter-spacing:-.03em}.section-head h2 small{display:block;margin-top:8px;color:var(--accent);font:700 clamp(11px,1.2vw,15px)/1.2 Outfit,sans-serif;letter-spacing:.18em;text-transform:uppercase}.section-count{white-space:nowrap;color:var(--muted);font-size:12px}.property-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}.property-card{background:var(--white);border:1px solid rgba(13,59,84,.1);border-radius:20px;overflow:hidden;box-shadow:0 12px 30px rgba(13,59,84,.07);display:flex;flex-direction:column;min-width:0;transition:transform .2s ease,box-shadow .2s ease,border-color .2s ease}.property-card:hover{transform:translateY(-4px);box-shadow:0 18px 38px rgba(13,59,84,.13);border-color:rgba(13,92,134,.2)}.property-photo-link{display:block}.property-photo{position:relative;aspect-ratio:16/10;background:linear-gradient(135deg,#d9e5e7,#b2c8cf);overflow:hidden}.property-photo img{width:100%;height:100%;object-fit:cover;display:block;transition:transform .22s ease}.property-photo-link:hover img{transform:scale(1.035)}.property-photo-empty{height:100%;display:grid;place-items:center;color:var(--muted);font-size:13px}.property-badge{position:absolute;left:11px;top:11px;padding:5px 9px;border-radius:999px;background:rgba(255,255,255,.92);color:var(--brand);font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase}.property-body{padding:15px;display:flex;flex:1;flex-direction:column;align-items:flex-start}.property-label{font-size:9px;letter-spacing:.15em;text-transform:uppercase;color:var(--accent);font-weight:700}.property-body h3{margin:7px 0 5px;font:600 20px/1.08 Fraunces,serif}.property-body h3 a:hover{color:var(--brand)}.property-location,.property-details,.property-code,.property-condo{margin:0;color:var(--muted);font-size:12px;line-height:1.45}.property-details{margin-top:7px}.property-code{margin-top:7px;color:var(--brand);font-weight:700}.property-condo{margin-top:7px;color:#477282}.property-body strong{margin-top:15px;font-size:17px;color:var(--brand)}.property-actions{display:grid;grid-template-columns:1fr 1fr;gap:7px;width:100%;margin-top:14px}.property-link,.property-interest{display:flex;justify-content:center;align-items:center;min-height:38px;padding:8px 9px;border-radius:999px;font:700 11px Outfit,sans-serif;cursor:pointer}.property-link{border:1px solid rgba(13,92,134,.25);color:var(--brand)}.property-interest{border:0;background:#128c7e;color:#fff;box-shadow:0 7px 14px rgba(18,140,126,.17)}.property-link:hover{background:var(--soft)}.property-interest:hover{filter:brightness(1.08)}.empty{padding:34px;border:1px dashed var(--line);border-radius:18px;background:#fff;color:var(--muted);text-align:center}.contact{margin-top:72px;background:var(--brand);color:#fff;border-radius:28px;padding:32px;display:grid;grid-template-columns:1fr 1fr;gap:28px;box-shadow:0 20px 42px rgba(13,59,84,.12)}.contact-profile{display:flex;align-items:flex-start;gap:13px;margin:0 0 18px}.profile-photo{display:block;width:86px;height:108px;border-radius:15px;object-fit:cover;object-position:center 24%;border:3px solid rgba(255,255,255,.82);box-shadow:0 12px 26px rgba(0,0,0,.2)}.contact-profile strong{display:block;margin-top:4px;font-size:16px}.contact-profile small{display:block;margin-top:6px;color:rgba(255,255,255,.78);font-size:11px;line-height:1.45}.contact h2{font:600 38px/1 Fraunces,serif;margin:0 0 12px}.contact p{color:rgba(255,255,255,.8);line-height:1.6}.contact-form{display:grid;gap:10px}.contact-form input,.contact-form textarea{width:100%;border:1px solid rgba(255,255,255,.25);border-radius:10px;background:rgba(255,255,255,.1);color:#fff;padding:12px;font:14px Outfit,sans-serif}.contact-form input::placeholder,.contact-form textarea::placeholder{color:rgba(255,255,255,.68)}.contact-form textarea{min-height:88px;resize:vertical}.consent{display:flex;gap:8px;align-items:flex-start;font-size:12px;color:rgba(255,255,255,.78);line-height:1.4}.consent input{width:auto;margin-top:3px}.form-submit{border:0;border-radius:999px;background:var(--accent);color:#193c50;padding:12px 16px;font:700 13px Outfit,sans-serif;cursor:pointer}.form-message{min-height:20px;font-size:13px;color:#ffe3a4}footer{background:#092f47;color:#fff;padding:48px 24px 18px;border-top:5px solid var(--accent)}.footer-main{width:min(1180px,100%);margin:auto;display:grid;grid-template-columns:1.5fr 1fr 1fr 1fr;gap:32px}.footer-logo{font:600 25px/1.05 Fraunces,serif}.footer-logo em{color:var(--accent);font-style:italic}.footer-text{max-width:310px;color:rgba(255,255,255,.7);font-size:13px;line-height:1.6;margin:14px 0 18px}.footer-title{color:var(--accent);font-size:10px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;margin-bottom:13px}.footer-col a{display:block;color:rgba(255,255,255,.75);font-size:13px;padding:5px 0}.footer-col a:hover{color:#fff}.footer-wpp{display:inline-flex;align-items:center;gap:8px;background:#25d366;color:#fff!important;border-radius:999px;padding:10px 15px!important;font-weight:700!important}.footer-bottom{width:min(1180px,100%);margin:30px auto 0;padding-top:16px;border-top:1px solid rgba(255,255,255,.14);display:flex;justify-content:space-between;gap:12px;color:rgba(255,255,255,.55);font-size:11px}.floating-wpp{position:fixed;right:22px;bottom:22px;z-index:50;display:inline-flex;align-items:center;gap:8px;background:#25d366;color:#fff;padding:12px 16px;border-radius:999px;font-size:13px;font-weight:700;box-shadow:0 12px 28px rgba(37,211,102,.35);transition:transform .2s}.floating-wpp:hover{transform:translateY(-2px)}.floating-wpp svg{width:20px;height:20px;fill:currentColor}@media(max-width:1000px){.property-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.footer-main{grid-template-columns:1.4fr 1fr 1fr}}@media(max-width:760px){.nav{padding:10px 14px}.brand-text{display:none}.nav-links{gap:7px;font-size:11px}.nav-links a:not(.nav-cta){display:inline;padding:7px 3px}.nav-cta{padding:8px 10px;font-size:11px}.hero{min-height:300px}.hero-bg{opacity:.44;filter:saturate(.82)}.hero-inner{padding:44px 18px 24px}.hero h1{max-width:390px;font-size:clamp(32px,9.5vw,50px);line-height:1}.hero-title-clean{gap:5px}.hero-title-clean span+span{font-size:.7em}.hero p{max-width:390px;font-size:14px;line-height:1.5}.hero-actions{margin-top:18px}.hero-action{min-height:40px;padding:9px 13px;font-size:12px}.hero-quick{gap:6px;margin-top:14px}.hero-quick span{font-size:10px;padding:6px 8px}.main{padding:26px 16px 96px}.search-panel{margin:-30px 0 28px;padding:15px 14px;border-radius:20px}.search-head{display:block}.search-head h2{font-size:24px}.search-head p{margin-top:7px}.search-form{grid-template-columns:1fr;gap:8px}.search-form input,.search-form select{min-height:48px}.search-form button{min-height:48px}.category-nav{margin-bottom:14px;padding-right:1px}.category-chip{min-width:156px;padding:11px 12px}.feature-strip{grid-template-columns:1fr;gap:4px;margin-bottom:12px;padding:10px}.feature-item{padding:6px 4px}.feature-icon{flex-basis:32px;width:32px;height:32px}.property-section{padding:36px 0 28px}.property-section+.property-section{margin-top:12px}.property-section.section-tone-soft{margin-left:-16px;margin-right:-16px;padding:34px 16px 34px;border-radius:22px}.section-head{align-items:flex-start;display:block}.section-count{display:block;margin-top:10px}.property-body{padding:12px}.property-body h3{font-size:18px}.property-actions{grid-template-columns:1fr}.intent-row{grid-template-columns:1fr;gap:10px;margin:32px 0 0}.intent-row a{padding:15px 16px}.intent-row strong{font-size:22px}.property-grid{grid-template-columns:1fr 1fr;gap:12px}.contact{grid-template-columns:1fr;padding:24px 18px}.contact h2{font-size:33px}.footer-main{grid-template-columns:1fr 1fr;gap:26px 20px}.footer-col:first-child{grid-column:1/-1}.footer-bottom{flex-direction:column;gap:5px}.floating-wpp{right:14px;bottom:78px;padding:13px 15px}.floating-wpp span{display:none}}@media(max-width:420px){.nav-links{gap:4px}.nav-links a:not(.nav-cta){font-size:10px}.nav-cta{padding:7px 8px}.hero h1{font-size:32px}.hero-title-clean span+span{font-size:.68em}.hero-quick span:nth-child(2){display:none}.property-grid{grid-template-columns:1fr}.category-chip{min-width:148px}.footer-main{grid-template-columns:1fr}.footer-col:first-child{grid-column:auto}}
</style>
</head>
<body>
<header class="site-header"><nav class="nav"><a class="brand" href="${landingUrl}" aria-label="Início do site do corretor parceiro">${logo}</a><div class="nav-links"><a href="${landingUrl}">Início</a><a href="${fullListingUrl}">Imóveis</a><a href="${contactUrl}">Contato</a><a class="nav-cta" href="${esc(wpp)}" target="_blank" rel="noopener nofollow">WhatsApp</a></div></nav></header>
...
${categoryNav}${featureStrip}<section id="imoveis" class="property-sections">${sectionMarkup}</section>
...
<footer><div class="footer-main"><div class="footer-col"><div class="footer-logo">${esc(name)}</div><p class="footer-text">Site profissional conectado ao catálogo do Condomínios na Praia, com imóveis selecionados e atendimento personalizado no Rio Grande do Sul.</p><a class="footer-wpp" href="${esc(wpp)}" target="_blank" rel="noopener nofollow">WhatsApp do corretor</a></div><div class="footer-col"><div class="footer-title">Navegar</div><a href="#buscar">Buscar imóveis</a><a href="${fullListingUrl}">Todos os imóveis</a><a href="${contactUrl}">Falar com o corretor</a><a href="${BASE}/">Portal Condomínios na Praia</a></div><div class="footer-col"><div class="footer-title">Atendimento</div><span>${esc(site.creci ? `CRECI ${site.creci}` : 'Atendimento imobiliário')}</span><span>${esc(site.cidade || 'Rio Grande do Sul')}</span>${site.email ? `<a href="mailto:${esc(site.email)}">${esc(site.email)}</a>` : '<span>Atendimento pelo WhatsApp</span>'}</div><div class="footer-col"><div class="footer-title">Informações</div><a href="${BASE}/politica-privacidade/">Política de Privacidade</a><a href="${BASE}/termos/">Termos de Uso</a><a href="${BASE}/contato/">Contato do portal</a></div></div><div class="footer-bottom"><span>© 2026 ${esc(name)} · Site parceiro Condomínios na Praia</span><span>Rio Grande do Sul · Brasil</span></div></footer>
...
<script type="application/json" id="partner-config">${jsonSafe(JSON.stringify({ wpp, name }))}</script>
...
const PARTNER_CONFIG=JSON.parse(document.getElementById('partner-config').textContent||'{}');const PARTNER_WPP=String(PARTNER_CONFIG.wpp||'');const PARTNER_NAME=String(PARTNER_CONFIG.name||'');
const leadForm=document.getElementById('lead-form');const message=document.getElementById('form-message');
function h(value){return String(value??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#039;'}[c]));}
function cityKey(value){return String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();}
function clientCard(p){const details=[p.bedrooms&&p.bedrooms+' quartos',p.suites&&p.suites+' suítes',p.area&&p.area+' m²'].filter(Boolean).join(' · ');const loc=[p.neighborhood,p.city].filter(Boolean).join(' · ');const interestText=encodeURIComponent('Olá, '+PARTNER_NAME+'! Tenho interesse no imóvel '+p.title+(p.code?', código '+p.code:'')+'. '+p.url);const interestUrl=PARTNER_WPP+'?text='+interestText;return '<article class="property-card property-card-compact"><a class="property-photo-link" href="'+h(p.url)+'"><div class="property-photo">'+(p.photo?'<img src="'+h(p.photo)+'" alt="'+h(p.title)+'" loading="lazy" decoding="async">':'<div class="property-photo-empty">Imagem em atualização</div>')+'</div></a><div class="property-body"><span class="property-label">'+h(p.type||'Imóvel')+'</span><h3><a href="'+h(p.url)+'">'+h(p.title)+'</a></h3><p class="property-location">'+h(loc||'Rio Grande do Sul')+'</p>'+(details?'<p class="property-details">'+h(details)+'</p>':'')+(p.code?'<p class="property-code">Código do imóvel: '+h(p.code)+'</p>':'')+(p.cond&&p.cond.name?'<p class="property-condo">Condomínio: '+h(p.cond.name)+'</p>':'')+'<strong>'+h(p.price?'R$ '+p.price:'Consulte o valor')+'</strong><div class="property-actions"><a class="property-link" href="'+h(p.url)+'">Ver imóvel</a><a class="property-interest" href="'+h(interestUrl)+'" target="_blank" rel="noopener nofollow">Tenho interesse</a></div></div></article>';}
...
leadForm.addEventListener('submit',async(e)=>{e.preventDefault();const data=Object.fromEntries(new FormData(leadForm));data.origem_url=location.href;data.consentimento_lgpd=Boolean(leadForm.querySelector('[name=consentimento_lgpd]').checked);message.textContent='Enviando...';try{const response=await fetch('/parceiro-lead',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(data)});const out=await response.json();if(!response.ok)throw new Error(out.error||'Não foi possível enviar.');leadForm.reset();message.textContent='Recebemos seus dados. O corretor entrará em contato.';}catch(error){message.textContent=error.message||'Não foi possível enviar agora.';}});
</script>
</body>
</html>`;
}
