(function(){
  ...
  const css=`
    ...
    .pml-fav-nav .heart{font-size:17px;line-height:1;color:#c79a3a}.pml-fav-nav .count{min-width:18px;height:18px;padding:0 5px;border-radius:99px;background:#0d3b54;color:#fff;font-size:10px;display:inline-flex;align-items:center;justify-content:center}
    .pml-fav-card-btn{position:absolute;top:12px;right:12px;z-index:4;width:38px;height:38px;border:1px solid rgba(255,255,255,.7);border-radius:50%;background:rgba(255,255,255,.94);color:#0d3b54;box-shadow:0 5px 16px rgba(13,59,84,.2);font:400 24px/1 'Outfit',sans-serif;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:transform .18s,background .18s,color .18s}
    .pml-fav-card-btn:hover{transform:scale(1.06);background:#fff}.pml-fav-card-btn.active{background:#0d3b54;color:#e0b34d;border-color:#0d3b54}.pml-fav-card-btn:focus-visible,.pml-fav-detail-btn:focus-visible,.pml-fav-nav:focus-visible,.pml-fav-account:focus-visible{outline:3px solid rgba(31,181,196,.4);outline-offset:2px}
    .pml-fav-detail-btn{display:inline-flex;align-items:center;justify-content:center;gap:8px;border:1px solid rgba(13,59,84,.18);background:#fff;color:#0d3b54;border-radius:100px;padding:9px 14px;font:600 12px/1 'Outfit',sans-serif;cursor:pointer;transition:all .18s;margin-left:auto}.pml-fav-detail-btn:hover{border-color:#1fb5c4;background:#e8f7f8}.pml-fav-detail-btn.active{background:#0d3b54;color:#e0b34d;border-color:#0d3b54}
  ...
  function refreshButtons(){
    document.querySelectorAll('[data-fav-id]').forEach(function(btn){
      ...
      btn.innerHTML='<span aria-hidden="true">'+(active?'♥':'♡')+'</span><span class="label">'+(active?'Salvo':'Favoritar')+'</span>';
  ...
  function updateAccountUi(){
    ...
    document.querySelectorAll('[data-pml-account]').forEach(function(btn){const label=user?('Olá, '+esc(name||'cliente')):'Entrar';if(btn.id==='pml-fav-mobile-account')btn.innerHTML='<span class="mob-footer-ico">'+(user?'●':'♡')+'</span><span class="mob-footer-lbl label">'+label+'</span>';else btn.innerHTML='<span aria-hidden="true">'+(user?'●':'♙')+'</span><span class="label">'+label+'</span>';});
  }
  ...
  async function renderFavoritesPage(){
    ...
    if(!state.session){root.innerHTML='<div class="pml-fav-page-card"><div class="pml-fav-empty"><h2>Entre para ver seus favoritos</h2><p>Faça seu cadastro ou login para acessar os imóveis que você salvou.</p><button type="button" class="pml-fav-primary" data-pml-account>Entrar ou criar cadastro</button></div></div>';updateAccountUi();return;}
    root.innerHTML='<div class="pml-fav-page-card"><div class="pml-fav-page-actions"><span class="pml-fav-badge">'+state.favoriteIds.size+' '+(state.favoriteIds.size===1?'imóvel salvo':'imóveis salvos')+'</span></div><div id="pml-fav-page-content"><div class="pml-fav-loading">Carregando seus imóveis...</div></div></div>';
